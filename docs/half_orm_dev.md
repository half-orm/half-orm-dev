...
### Reference Data Management (model/data-X.Y.Z.sql)

#### Purpose and Lifecycle

`model/data-X.Y.Z.sql` is a single, unrestricted `pg_dump --data-only`
snapshot generated for every production version. It contains **both**
half-orm-dev's own tracking data (`half_orm_meta.*`) **and** all
application data - there is no separate file for each. It replaces the
older two-file split (`metadata-X.Y.Z.sql` for tracking only,
`seed-X.Y.Z.sql` proposed separately for application reference data)
that earlier drafts of this document described but that was never
implemented that way.

```bash
model/
├── schema-1.3.4.sql   # Database structure (tables, views, functions) only
└── data-1.3.4.sql     # ALL data: half_orm_meta.* + every application table

# Only schema.sql is a symlink; its target names the version, and
# data-X.Y.Z.sql is deduced from that same version - no symlink of its own
├── schema.sql → schema-1.3.4.sql  # ← Version indicator
```

Restoration (`restore_database_from_schema()`, used by `patch apply` and
`clone`) always loads `schema.sql` then, if present, the matching
`data-X.Y.Z.sql` - automatically, no configuration needed. A missing data
file is not an error (backward compatibility with pre-1.0.0a33 projects).

#### What Belongs in a Patch vs. `bootstrap/`

There is no separate "seed file" step or config to author reference data
into. Two places remain:

**Idempotent DML in a patch** - for anything that should exist on
**every** instance (application roles, reference lookup tables, feature
flags, system configuration):
```sql
...
```
Once that patch ships, its data is captured automatically in the next
`data-X.Y.Z.sql` generated at production promotion - every future
`patch apply` and `clone` gets it for free.

**`bootstrap/` scripts** - reserved for data that is genuinely specific
to *one* instance (an initial admin account, a per-site secret). They run
exactly once, at `clone`, and are never captured in `data-X.Y.Z.sql`.
