...
class PatchManager:
    ...
    def merge_patch(self) -> dict:
        ...
        try:
            ...
            if not self._auto_resolve_generated_conflicts(e, patch_branch, release_branch):
                # Leave the release branch as it was found rather than
                # mid-merge with a conflicted index, and hand the
                # developer back their patch branch.
                self._abort_merge_in_progress()
                try:
                    self._repo.hgit.checkout(current_branch)
                except GitCommandError:
                    pass
                ...
                raise PatchManagerError(
                    ...
                    f"The merge was aborted, {release_branch} is unchanged.\n"
                    f"Resolve the conflict on {patch_branch}, then retry:\n"
                    f"  git merge {release_branch}\n"
                    f"  # resolve, commit, push\n"
                    f"  half_orm dev patch merge"
                )
    ...
    def _abort_merge_in_progress(self) -> None:
        """
        Abort a merge left in progress, if there is one.

        A merge that stops on a conflict leaves a conflicted index
        behind, and every later checkout then fails with "you need to
        resolve your current index first". Without this, a conflict
        during validation stranded the developer on the temporary
        ho-validate branch, mid-merge, with the branch left undeleted -
        the cleanup could neither switch back nor remove it.
        """
        try:
    ...
    def _validate_patch_before_merge(
        ...
        try:
            ...
            try:
                ...
                if not self._auto_resolve_generated_conflicts(e, patch_branch):
                    raise PatchManagerError(
                        ...
                        f"Nothing was committed and the validation branch is "
                        f"discarded - you are back on {patch_branch}.\n"
                        f"Resolve the conflict there, then retry:\n"
                        f"  git merge {release_branch}\n"
                        f"  # resolve, commit, push\n"
                        f"  half_orm dev patch merge"
                    )
            ...
            try:
                # A conflicted merge has to be undone first: with the
                # index left unmerged, both the checkout below and the
                # deletion of the temp branch fail.
                self._abort_merge_in_progress()
