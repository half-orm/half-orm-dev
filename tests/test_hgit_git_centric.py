...
class TestImmediateBranchReservation(TestHGitGitCentricWorkflow):
    ...
    def test_set_branch_with_immediate_push(self):
        ...
        # Mock the git operations to avoid real git commands
        mock_repo_instance = MagicMock()
        mock_git = MagicMock()
        mock_repo_instance.git = mock_git
        mock_repo_instance.active_branch = MagicMock()
        mock_repo_instance.active_branch.__str__ = MagicMock(return_value='hop_main')
        mock_repo_instance.create_head = MagicMock()
        mock_repo_instance.heads = MagicMock()
        
        # Create HGit with mocked repo
        with patch.object(HGit, '_HGit__post_init'):
            hgit = HGit(self.mock_repo)
            hgit._HGit__git_repo = mock_repo_instance
        ...
        with patch.object(hgit, 'immediate_branch_push') as mock_push:
            with patch.object(hgit, 'add') as mock_add:
                with patch.object(hgit, 'cherry_pick_changelog') as mock_cherry:
                    # Mock changelog file
                    self.mock_repo.changelog = MagicMock()
                    self.mock_repo.changelog.file = '/fake/changelog'
                    
                    # Execute the method we're testing
                    hgit.set_branch('1.2.3')
                    
                    # Verify immediate_branch_push was called
