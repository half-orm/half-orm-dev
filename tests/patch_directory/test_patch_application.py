...
class TestApplyPatchFiles:
    ...
    def test_apply_patch_files_sql_and_python(self, patch_directory, mock_database):
        ...
        applied_files = patch_dir.apply_patch_files("456-mixed", mock_database)
    ...
    def test_apply_patch_files_empty_patch(self, patch_directory, mock_database):
        ...
        applied_files = patch_dir.apply_patch_files("456-empty", mock_database)

        # Should return empty list
        assert applied_files == []
        assert mock_database.execute_query.call_count == 0

    def test_apply_patch_files_mixed_file_types(self, patch_directory, mock_database):
        """Test applying patch with mixed file types (some ignored)."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create patch with various file types
        patch_path = schema_patches_dir / "456-mixed-types"
        patch_path.mkdir()
        ...
        applied_files = patch_dir.apply_patch_files("456-mixed-types", mock_database)
    ...
    def test_apply_patch_files_nonexistent_patch(self, patch_directory, mock_database):
        """Test applying nonexistent patch."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        with pytest.raises(PatchDirectoryError, match="Cannot apply invalid patch"):
            patch_dir.apply_patch_files("999-nonexistent", mock_database)

    def test_apply_patch_files_invalid_patch(self, patch_directory, mock_database):
        """Test applying invalid patch (file instead of directory)."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create file instead of directory
        patch_file = schema_patches_dir / "456-invalid"
        patch_file.write_text("Not a directory")

        with pytest.raises(PatchDirectoryError, match="Cannot apply invalid patch"):
            patch_dir.apply_patch_files("456-invalid", mock_database)

    def test_apply_patch_files_sql_execution_error(self, patch_directory, mock_database):
        """Test SQL execution error during patch application."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create patch with SQL file
        patch_path = schema_patches_dir / "456-sql-error"
        patch_path.mkdir()
        ...
        with pytest.raises(PatchDirectoryError, match="SQL execution failed in bad.sql"):
            patch_dir.apply_patch_files("456-sql-error", mock_database)

    def test_apply_patch_files_python_execution_error(self, patch_directory, mock_database):
        """Test Python execution error during patch application."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create patch with failing Python script
        patch_path = schema_patches_dir / "456-python-error"
        patch_path.mkdir()
        (patch_path / "bad_script.py").write_text("raise ValueError('Python error')")

        with pytest.raises(PatchDirectoryError, match="Python execution failed in bad_script.py"):
            patch_dir.apply_patch_files("456-python-error", mock_database)

    def test_apply_patch_files_empty_sql_file(self, patch_directory, mock_database):
        """Test applying patch with empty SQL file (should be skipped)."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create patch with empty SQL file
        patch_path = schema_patches_dir / "456-empty-sql"
        patch_path.mkdir()
        (patch_path / "empty.sql").write_text("")
        (patch_path / "whitespace.sql").write_text("   \n  \t  ")
        (patch_path / "valid.sql").write_text("SELECT 1;")

        applied_files = patch_dir.apply_patch_files("456-empty-sql", mock_database)
    ...
    def test_apply_patch_files_lexicographic_order(self, patch_directory, mock_database):
        ...
        applied_files = patch_dir.apply_patch_files("456-order-test", mock_database)
...
class TestExecuteSqlFile:
    ...
    def test_execute_sql_file_success(self, patch_directory, mock_database):
        ...
        # Execute SQL file
        patch_dir._execute_sql_file(sql_file, mock_database)

        # Should call database execute_query once
        assert mock_database.execute_query.call_count == 1
        mock_database.execute_query.assert_called_with("CREATE TABLE test (id INTEGER);")

    def test_execute_sql_file_utf8_content(self, patch_directory, mock_database):
    ...
    def test_execute_sql_file_empty_file(self, patch_directory, mock_database):
    ...
    def test_execute_sql_file_database_error(self, patch_directory, mock_database):
        ...
        # Should raise PatchDirectoryError
        with pytest.raises(PatchDirectoryError, match="SQL execution failed in error.sql"):
            patch_dir._execute_sql_file(sql_file, mock_database)

    def test_execute_sql_file_read_error(self, patch_directory, mock_database):
        ...
        # Should raise PatchDirectoryError
        with pytest.raises(PatchDirectoryError, match="SQL execution failed in nonexistent.sql"):
            patch_dir._execute_sql_file(sql_file, mock_database)
...
class TestExecutePythonFile:
    ...
    def test_execute_python_file_success(self, patch_directory):
        ...
        with mock_patch('subprocess.run') as mock_run:
            ...
            # Execute Python file
            patch_dir._execute_python_file(py_file)

            # Should call subprocess.run
            assert mock_run.call_count == 1

    def test_execute_python_file_with_output(self, patch_directory, capsys):
    ...
    def test_execute_python_file_error(self, patch_directory):
        ...
        with mock_patch('subprocess.run') as mock_run:
            ...
            # Should raise PatchDirectoryError
            with pytest.raises(PatchDirectoryError, match="Python execution failed in error.py"):
                patch_dir._execute_python_file(py_file)

    def test_execute_python_file_subprocess_error(self, patch_directory):
        ...
        with mock_patch('subprocess.run') as mock_run:
            ...
            # Should raise PatchDirectoryError
            with pytest.raises(PatchDirectoryError, match="Failed to execute Python file"):
                patch_dir._execute_python_file(py_file)

    def test_execute_python_file_working_directory(self, patch_directory):
        """Test Python file execution uses correct working directory."""
        patch_dir, repo, temp_dir, schema_patches_dir = patch_directory

        # Create Python file in patch directory
        patch_path = schema_patches_dir / "456-working-dir"
        patch_path.mkdir()
